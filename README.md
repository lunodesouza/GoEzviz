...
- Camera audio, volume control, and PTZ (pad, speed, digital zoom)
- **Digital zoom** on the PTZ pad (the H9c does not expose optical zoom over ONVIF)
...
- **Keep display on**: optional checkbox that prevents the notebook from sleeping while GoEzviz is open
...
On a PTZ lens, use the on-screen pad to pan/tilt and pick Slow / Medium / Fast for how far each nudge moves. Use the zoom buttons to enlarge the picture (1x–4x digital zoom). Click **★**, enter a name, and save a favorite; choose it from the list to go back to that position.
